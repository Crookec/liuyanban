...
<div id="second">
<table>
<tr><td height="100" width="280px"><span style="color:white;font-size:18px;font-weight:bold;"><%=user.get_uname() %></span></td></tr>
...
<th width="700"><%=pinfo.getText() %></th></tr>
<%
...
<td width="800"><%=replyinfo.getText() %></td></tr>
<% }}%>
...
<button type="button" style="height:30px;background-color:black;" onClick="REPLY()">
<span style="font-weight:bold;color:white;font-size:14px;"> 回复</span> 
</button>
...
function REPLY(){
	...
	if(text=="")
	 ...
	 loadXMLDoc("reply_add_code.jsp?uphone=" +<%=uphone%> +"&text="+text
				+"&pid="+<%=pid%>+"&timestamp="+ new Date().getTime(), proc_add );
}

function proc_add(){
	if (xmlhttp.readyState==4&&xmlhttp.status==200){
		alert("发表回复成功");
		window.location.href="Message.jsp?uphone="+<%=uphone%>+"&pid="+<%=pid%>;
	}
}
</script>
</html>